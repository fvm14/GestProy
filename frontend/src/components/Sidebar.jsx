...
const Sidebar = ({ children }) => {
  ...
  // ⚠️ NO INCLUIMOS '/caso/:id' porque esa vista se accede solo al hacer clic desde /casos
  const items = [
    { text: 'Dashboard', icon: <DashboardIcon />, path: '/dashboard' },
    { text: 'Cambiar nombre', icon: <EditIcon />, path: '/cambiar-nombre' },
    { text: 'Casos clínicos', icon: <AssignmentIcon />, path: '/casos' },
    { text: 'Nuevo caso', icon: <AddCircleIcon />, path: '/nuevo-caso' }
  ];

  return (
    <Box sx={{ display: 'flex' }}>
      <AppBar position="fixed" sx={{ zIndex: 1300, bgcolor: '#5D3FD3' }}>
        <Toolbar>
