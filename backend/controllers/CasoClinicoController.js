...
router.post('/', upload.single('archivo'), (req, res) => {
  const { codigo, paciente_id, disenador_id, doctor_id, notas } = req.body;
  ...
  const query = `
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `;
  db.run(query, [codigo, paciente_id, disenador_id, doctor_id, notas, archivo], function (err) {
    if (err) {
...
/// Obtener caso por ID (con nombres relacionados)
...
router.put('/:id', upload.single('archivo'), (req, res) => {
  const id = req.params.id;
  const { notas, progreso, ver_prototipo, estado } = req.body;
  const archivo = req.file ? req.file.filename : null;
